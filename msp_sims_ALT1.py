...
# L is length of simulated region in bases (set to 1M here)
# 
# Here we are looking at admixed segment length distributions, f4 tests, and D tests results.
# The model we are testing is ALT1B, with an admixture event from Neanderthal into the shared ancestor of all Non-Africans, a Denisovan admixture event into the ancestor of Papuans and Ayta, and a Denisovan admixture event private to Ayta. The difference from ALT1 is that there is an additional admixture event from East Asian into Ayta.
